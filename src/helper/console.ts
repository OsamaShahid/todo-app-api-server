...
const myFormat = winston.format.printf((info: any) => `[${new Date(info.timestamp).toISOString()}] ${info.message}`);

const getConsoleFormat = () => {

  return winston.format.combine(winston.format.timestamp(), winston.format.colorize({ all: true }), myFormat);
...
const getLogger = (type: string) => (...args: any) => {
